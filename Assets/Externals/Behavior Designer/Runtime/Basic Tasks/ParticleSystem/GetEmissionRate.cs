...
namespace BehaviorDesigner.Runtime.Tasks.Basic.UnityParticleSystem
{
    ...
    public class GetEmissionRate : Action
    {
        ...
        public override TaskStatus OnUpdate()
        {
            ...
            var rate = particleSystem.emission.rate;
            if (rate.constantMin == rate.constantMax)
            {
                storeResult.Value = rate.constantMax;
            }
            else
            {
                Debug.LogWarning("ParticleSystem emission rate max is not equal mix");
            }
